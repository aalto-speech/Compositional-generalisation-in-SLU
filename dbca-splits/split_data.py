...
import random
random.seed(42)
from freq_mats import save_struct

from data_analysis import divergence
import argparse

AUDIO_PATH = "/m/teamwork/t40511_asr/c/SLURP/audio/slurp_real"
ORIGINAL_DATA_PATH = "/m/teamwork/t40511_asr/c/SLURP/speechbrain_data/original_splits"
METADATA_DIR = "/m/teamwork/t40511_asr/c/SLURP/metadata"
...
def label_distributions(data, disregard_order=False, weight_by_num_recs=True):
...
def distribution_similarity(train, dev, test, metadata, verbose=False):
    """
    ...
    train_distribs = label_distributions(train)
    dev_distribs = label_distributions(dev)
    test_distribs = label_distributions(test)
    ...
    for label_type in label_types:
        ...
        print("Number of lables:", len(all_labels[label_type]))
        ...
        train_labels = train_distribs[label_type].keys()
        test_labels = test_distribs[label_type].keys()
        dev_labels = dev_distribs[label_type].keys()
...
def recs_to_original_metadata(sb_file, metadata, rec2slurpid):
    ...
    for rec_file in jsondict:
        slurpid = rec2slurpid[rec_file]
        if slurpid not in new_metadata:
            new_metadata[slurpid] = metadata[slurpid]
            new_metadata[slurpid]['recordings'] = [{'file': rec_file}]
        else:
            new_metadata[slurpid]['recordings'].append({'file': rec_file})
    return new_metadata
...
def get_orig_metadata():
    metadata = {}
    for subset in ['train', 'dev', 'test']:
        with open(f"{METADATA_DIR}/{subset}.json", "r", encoding="utf-8") as f:
            for line in f.readlines():
                line = json.loads(line)
...
def main(args):
    ...
    if args.split_data:
        from divide import FromEmptySets
    ...
    if args.analyse_splits:
        if args.sb_data_dir and os.path.isdir(args.sb_data_dir):
            ...
            if os.path.exists(os.path.join(args.sb_data_dir, "train.txt")):
                tr_dv_te = []
