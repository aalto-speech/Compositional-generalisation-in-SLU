...
class SER(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        # resample to 16kHz
        sig = self.hparams.resampler(sig)
...
def data_prep(data_folder, hparams):
    ...
    train_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "difficult_splits/train_new.json"), replacements={"data_root": data_folder})
    valid_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "difficult_splits/dev_new.json"), replacements={"data_root": data_folder})
    test_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "difficult_splits/test_new.json"), replacements={"data_root": data_folder})
    ...
    @sb.utils.data_pipeline.takes("scenario")
    @sb.utils.data_pipeline.provides("labels_encoded")
    def text_pipeline(scenario):
        labels_encoded = hparams["label_encoder"].encode_sequence_torch([scenario])
        yield labels_encoded

    sb.dataio.dataset.add_dynamic_item(datasets, text_pipeline)
    
    hparams["label_encoder"].update_from_didataset(train_data, output_key="scenario")
    hparams["label_encoder"].update_from_didataset(valid_data, output_key="scenario")
    hparams["label_encoder"].update_from_didataset(test_data, output_key="scenario")
...
def main(device="cuda"):
    ...
    # Dataset creation
    train_data, valid_data, test_data = data_prep("/m/teamwork/t40511_asr/c/SLURP/speechbrain_data/scenario_action/", hparams)
