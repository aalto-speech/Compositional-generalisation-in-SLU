...
def data_prep(data_folder, hparams):
    ...
    train_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "easy_splits/train.json"), replacements={"data_root": data_folder})
    valid_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "easy_splits/dev.json"), replacements={"data_root": data_folder})
    test_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "easy_splits/test.json"), replacements={"data_root": data_folder})
    ...
    def audio_pipeline(data_path_1, data_path_2):
        ...
        # combine the signals
        silence_length = int(hparams["sample_rate"] * 0.5) # add 0.5 seconds silence
        silence = torch.zeros(silence_length)
        sig = torch.cat([sig_1, silence, sig_2], dim=0)
        return sig
...
def main(device="cuda"):
    ...
    # Dataset creation
    train_data, valid_data, test_data = data_prep("/m/teamwork/t40511_asr/c/SLURP/speechbrain_data/scenario_two_actions", hparams)
